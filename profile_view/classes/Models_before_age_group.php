	...
	public function searchModels($type,$gend,$age=NULL,$eth=NULL){
		...
		if (!$this->db_connection->connect_errno) {
			...
			if($eth!=NULL){
				...
				if($age!=NULL){
					$stmt = $this->db_connection->query("SELECT Resource_ID,Resource_Type,Sub_Category,First_Name,Gender,Age,Ethnicity,Client_ID,Height,Bust,Waist,Hips,HairColor,ShoesSize,EyesColor,Status,Date_Created,Created_By,Date_Modified,Modified_By FROM Smart_FLC_Resource_Details WHERE Gender='$gend' && Age='$age' && Resource_Type LIKE '%$type%' ".$page_query);
				}
				else{
	...
	public function paginateResult($type,$gend,$age=NULL,$eth=NULL){
		...
		if (!$this->db_connection->connect_errno) {
			if($eth!=NULL){
				$eth_str=implode("', '", $eth);
				if($age!=NULL){
					$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE Gender='$gend' && Age='$age' && Ethnicity IN('$eth_str') && Resource_Type LIKE '%$type%' ");
				}
				else{
				...
				if($age!=NULL){
					$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE Gender='$gend' && Age='$age' && Resource_Type LIKE '%$type%' ");
				}
				else{
