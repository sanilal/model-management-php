...
*/
	...
	public function searchModels($type,$gend,$age=NULL,$eth=NULL,$name=""){
		...
		if (!$this->db_connection->connect_errno) {
			...
			if($eth!=NULL){
				...
				if($age!=NULL){
					$stmt = $this->db_connection->query("SELECT Resource_ID,Resource_Type,Sub_Category,First_Name,Gender,Age,Ethnicity,Client_ID,Height,Bust,Waist,Hips,HairColor,ShoesSize,EyesColor,SkinColor,Status,Date_Created,Created_By,Date_Modified,Modified_By,In_Town_Status FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' && Age BETWEEN ".$age." && (".$eth_str_new.") && Resource_Type LIKE '%$type%' ".$page_query);
				}
				else{
					$stmt = $this->db_connection->query("SELECT Resource_ID,Resource_Type,Sub_Category,First_Name,Gender,Age,Ethnicity,Client_ID,Height,Bust,Waist,Hips,HairColor,ShoesSize,EyesColor,SkinColor,Status,Date_Created,Created_By,Date_Modified,Modified_By,In_Town_Status FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' && (".$eth_str_new.") && Resource_Type LIKE '%$type%' ".$page_query);
				}
			}
			else{
				if($age!=NULL){
					$stmt = $this->db_connection->query("SELECT Resource_ID,Resource_Type,Sub_Category,First_Name,Gender,Age,Ethnicity,Client_ID,Height,Bust,Waist,Hips,HairColor,ShoesSize,EyesColor,SkinColor,Status,Date_Created,Created_By,Date_Modified,Modified_By,In_Town_Status FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' && Age BETWEEN ".$age." && Resource_Type LIKE '%$type%' ".$page_query);
				}
				else{
					$stmt = $this->db_connection->query("SELECT Resource_ID,Resource_Type,Sub_Category,First_Name,Gender,Age,Ethnicity,Client_ID,Height,Bust,Waist,Hips,HairColor,ShoesSize,EyesColor,SkinColor,Status,Date_Created,Created_By,Date_Modified,Modified_By,In_Town_Status FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' && Resource_Type LIKE '%$type%' ".$page_query);
	...
	public function paginateResult($type,$gend,$age=NULL,$eth=NULL,$name=""){
		...
		if (!$this->db_connection->connect_errno) {
			...
			if($eth!=NULL){
				...
				$eth_str_new=substr($eth_str, 0, -2);
				if($age!=NULL){
					$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' && Age BETWEEN ".$age." && (".$eth_str_new.") && Resource_Type LIKE '%$type%' ".$page_query);
				}
				else{
					$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' && (".$eth_str_new.") && Resource_Type LIKE '%$type%' ".$page_query);
				}
			}
			else{
				if($age!=NULL){
					$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' && Age BETWEEN ".$age." && Resource_Type LIKE '%$type%' ".$page_query);
				}
				else{
					$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' && Resource_Type LIKE '%$type%' ".$page_query);
				}
			}
			return $stmt;
			$this->errors[] = "Sorry, Please go back and try again.";
	...
	public function searchSpModels($type,$name="",$cat=""){
		...
		if (!$this->db_connection->connect_errno) {
			...
			//
			if($cat!=""){
				$stmt = $this->db_connection->query("SELECT Resource_ID,Resource_Type,Sub_Category,First_Name,Gender,Age,Ethnicity,Client_ID,Height,Bust,Waist,Hips,HairColor,ShoesSize,EyesColor,SkinColor,Status,Date_Created,Created_By,Date_Modified,Modified_By,In_Town_Status FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Sub_Category LIKE '%$cat%' && Resource_Type LIKE '%$type%' ".$page_query);
			}
			else{
				$stmt = $this->db_connection->query("SELECT Resource_ID,Resource_Type,Sub_Category,First_Name,Gender,Age,Ethnicity,Client_ID,Height,Bust,Waist,Hips,HairColor,ShoesSize,EyesColor,SkinColor,Status,Date_Created,Created_By,Date_Modified,Modified_By,In_Town_Status FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Resource_Type LIKE '%$type%' ".$page_query);
			}
			return $stmt;
			$this->errors[] = "Sorry, Please go back and try again.";
		}else {
	...
	public function paginateSpResult($type,$name="",$cat=""){
		...
		if (!$this->db_connection->connect_errno) {
			...
			$page_query=$in_town_q."ORDER BY First_Name ASC";
			if($cat!=""){
				$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Resource_Type LIKE '%$type%' && Sub_Category LIKE '%$cat%' ".$page_query);
			}
			else{
				$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Resource_Type LIKE '%$type%' ".$page_query);
			}
			return $stmt;
			$this->errors[] = "Sorry, Please go back and try again.";
		}else {
	...
	public function searchHost($type,$gend,$age=NULL,$eth=NULL,$name="",$lang=NULL,$cat=NULL){
		...
		if (!$this->db_connection->connect_errno) {
			...
			//
			$stmt = $this->db_connection->query("SELECT Resource_ID,Resource_Type,Sub_Category,First_Name,Gender,Age,Ethnicity,Client_ID,Height,Bust,Waist,Hips,HairColor,ShoesSize,EyesColor,SkinColor,Status,Date_Created,Created_By,Date_Modified,Modified_By,In_Town_Status FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' ".$age_query." ".$eth_str_new." ".$lang_str_new." ".$cat_query." && Resource_Type LIKE '%$type%' ".$page_query);
			return $stmt;
			$this->errors[] = "Sorry, Please go back and try again.";
		}else {
			$this->errors[] = "Sorry, no database connection.";
	...
	///////////////////////////////////////////
	public function paginateHostResult($type,$gend,$age=NULL,$eth=NULL,$name="",$lang=NULL){
		$this->db_connection=db_connect();
		if (!$this->db_connection->connect_errno) {
			...
			$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' ".$age_query." ".$eth_str_new." ".$lang_str_new." && Resource_Type LIKE '%$type%' ".$page_query);
