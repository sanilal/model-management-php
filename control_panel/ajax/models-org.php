...
function get_Personal_Details($val){
	global $url;
	  $model=mysqli_fetch_object(mysqli_query($url,"select `Gender`, `Age`,`Resource_Type`,`In_Town_Status`,`Sub_Category`, `Ethnicity`, `Nationality` from `Smart_FLC_Resource_Details` WHERE Resource_ID='".$val."'"));
...
function get_Physical_Details($val){
	global $url;
	  ...
	  return '<p>Height: <b>'.$model->Height.'</b> &nbsp;&nbsp;&nbsp;&nbsp; Weight: <b>'.$model->Weight.'</b></p>
...
$columns = array(
	...
	array( 'db' => 'Cell_phone', 'dt' => 4,'formatter' => function( $d, $row ) {
				// var_dump($row);
