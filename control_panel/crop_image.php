...
//crop
  ...
  function checkCoords()
  {
	...
	if (parseInt($('#w').val())) {
		...
		$.ajax({
				  ...
				  success: function(message) {
					  ...
					  $extension_m_pos=strrpos($img_val, '.');
					  $croped_m_name = substr($img_val, 0, $extension_m_pos) . '-cr' . substr($img_val, $extension_m_pos);
	$croped_name = substr($trim_val, 0, $extension_pos) . '_cr' . substr($trim_val, $extension_pos);
					  ...
					  //$("#update_button").click();
					  new_img='<?php echo $croped_m_name; ?>';
					  $("#mail_edit_form").submit();
					  $("#model_edit_form").submit();
				 	 }
				  }
			});
	}
