...
if(isset($_POST['job_form_id'])){

	$from = 'no-reply@flcmodels.com';
	$subject = "Job Booking form - Flcmodels";
	$insertid=$_POST['job_form_id'];
	$job_res=mysqli_fetch_object(mysqli_query($url,"select * from `Smart_FLC_jobs` WHERE job_id=".$insertid));
	$ja_query=mysqli_query($url,"SELECT * FROM `Smart_FLC_job_assign` as j INNER JOIN `Smart_FLC_Resource_Details` as r ON j.model_id=r.Resource_ID WHERE job_id=".$insertid);
	$mail_check=0;
	...
	while($model=mysqli_fetch_object($ja_query)){
		...
		//var_dump($model); exit;
		//$to="augustianjoseph@gmail.com";
	if($to!="" && $model->job_assign_status=="Confirmed" && $model->client_check=='1' ){
		$model_id=$model->Resource_ID;
	$en_url=base64_encode("model_id=$model_id&job_id=$insertid");
	$message= '<html>
	   <body bgcolor=\"#DCEEFC\"> 
...
<a href="http://flcmodels.com/book_form/?q='.$en_url.'"><h3 style="font-family: Tahoma,Geneva,sans-serif;">CLick here to view form & agree the terms and conditions</h3></a>
	  </body>
	  </html>
	  ';
	  /*$headers  = "MIME-Version:1.0 \r\n";
	...
	if($mail_check==1){
		mysqli_query($url,"UPDATE `Smart_FLC_jobs` SET `form_mail`='1' WHERE job_id=".$insertid);
		$msg = "Book Form sent successfully to all the models confirmed on the job.";}
...
?>  
   ...
   <?php include_once('includes/footer-scripts.php'); ?>
	...
	function send_mail(obj){
		var c= confirm("Sending job offer mail to the selected models!");
		if(c){
			$(obj).parent().submit();
		}
	}
	function send_form(obj){
		var c= confirm("Sending job Booking form to the confirmed and client selected models!");
		if(c){
			$(obj).parent().submit();
		}
	}
