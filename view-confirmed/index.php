...
<body style=" background:#000; background-size:contain; margin:0 auto; padding:0; width:900px;" >
   <?php /*?> <div style="width:100%; text-align:center;" id="top">
        <img src="images/flc-bg.jpg" width="100%" />
    </div><?php */?>
    <div style="width:100%" id="print_div">
      ...
      <button type="button" onClick="screenshot()" class="btn btn-warning no-print" data-html2canvas-ignore>Save Page</button>
			...
			$cat_rext_arr=explode(":;",$job->model_cats); 
			 ...
			 foreach($cat_rext_arr as $cat_text){
				 $query=mysqli_query($url,"SELECT * FROM `Smart_FLC_job_assign` as j INNER JOIN `Smart_FLC_Resource_Details` as r ON j.model_id=r.Resource_ID WHERE job_id='".$job_id."' && job_assign_status='Confirmed' && model_cat='".$cat_id."'");
            ...
            function screenshot(){
				...
				html2canvas(document.getElementById("print_div"), 
				{
				  onrendered: function (canvas) {
    				...
    				pdf.save('JB<?php echo $job_id; ?>_models.pdf');﻿
					/*document.body.appendChild(canvas);
					pdf.addHTML(canvas, 0, 0, pdfConf, function() {
					  document.body.removeChild(canvas);
					  pdf.addPage();
					  pdf.save('JB<?php echo $job_id; ?>.pdf');
					  //callback();
					});*/
				  }
				});
