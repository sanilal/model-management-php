...
*/
	...
	public function paginateHostResult($type,$gend,$age=NULL,$eth=NULL,$name="",$lang=NULL,$cat=NULL){
		...
		if (!$this->db_connection->connect_errno) {
			...
			$stmt = $this->db_connection->query("SELECT count(Resource_ID) as total FROM Smart_FLC_Resource_Details WHERE First_Name LIKE '%$name%' && Gender='$gend' ".$age_query." ".$eth_str_new." ".$lang_str_new." ".$cat_query." && Resource_Type LIKE '%$type%' ".$page_query);
