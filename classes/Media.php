...
*/
	...
	private function updateMedia(){
		...
		if (empty($_POST['media_title'])) {
            ...
            if (!$this->db_connection->connect_errno) {
				...
				if($_FILES['media_img']['name']!=""){
					//var_dump($this->flc_clean($media_title)); exit();
					$path = $_FILES['media_img']['name'];
					$ext = pathinfo($path, PATHINFO_EXTENSION);
					$newname = $this->flc_clean($media_title).'_img.'.$ext;
					//var_dump($newname);
	...
	public function flc_clean($string) {
	...
	public function getMedia($val=NULL,$cat=NULL,$cat_name=NULL){
		...
		if (!$db_connection->connect_errno) {
			...
			if($val!=NULL){
				...
				$stmt = $db_connection->query("SELECT work_id, work_title,category_id, work_type,work_image,work_image2,work_image3,work_image4,work_image5,work_image6,work_image7,work_image8,work_image9,work_image10,work_link,work_order FROM our_works WHERE category_id=$id ORDER BY work_id desc");
